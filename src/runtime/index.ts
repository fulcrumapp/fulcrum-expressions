...
export interface WindowWithRuntime {
  [key: string]: any
  $$runtime: Runtime
...
export default class Runtime {
  ...
  constructor() {
    try {
      ...
      this.global = globalThis as unknown as WindowWithRuntime
    }
    ...
    const exportObject = (exportName: string) => {
      ...
      function wrapper(...wrapperArgs: any[]) {
        if (isFunction(object)) {
          ...
          return object.apply(functions, wrapperArgs)
        } else {
          return object
        }
      }
 ...
 */
  evaluateExpression = (context: any): ExpressionResult => {
    ...
    try {
      ...
      console.log(`JS ERROR : ${context.dataName} : ${ex?.toString?.() ?? String(ex)}`)

      this.variables[thisVariableName] = undefined

      return createResult(context.key, null, null, ex?.toString?.() ?? String(ex), true)
    }
  }
  ...
  invokeAsync(func: HostFunction, args: any[], callback: Function) {
    ...
    ;(func as Function).apply(this, args.concat([id]))
  }
