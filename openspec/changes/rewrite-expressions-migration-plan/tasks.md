...
## 1. 1 Adapter contract and CI gate foundation
...
## 2. 2 Side-by-side build enablement from the start
...
## 3. 3 Deterministic manifest routing and parity batches
...
## 4. 4 Host effects and lifecycle parity closure
...
- [ ] 4.3 Close discovered parity gaps in TS implementations or adapter plumbing, and verify all host/lifecycle parity suites pass without relaxing assertions.

## 5. 5 Promotion readiness and controlled default switch

- [ ] 5.1 Define the production promotion checklist requiring all per-slice gates (legacy/hybrid contracts, unchanged differential checks, focused migrated parity), and verify checklist completion is machine-verifiable in CI outputs.
- [ ] 5.2 Execute rollback drills for both paths (channel switch to full legacy, manifest flip back to legacy), and verify each rollback path restores expected behavior in test/staging.
- [ ] 5.3 Promote TS/hybrid channel to production default only after checklist and rollback drills pass, and verify post-switch smoke and contract checks remain green.
